...
function footerHtml() {
  return [
    ...
    '          <input class="footer-email-input" type="email" placeholder="' +
      escHtml(t("footer_email_placeholder")) +
      '" required>',
    '          <button type="submit" class="footer-email-btn">' +
      escHtml(t("footer_subscribe")) +
      "</button>",
    "        </form>",
    "      </div>",
    "    </div>",
...
function productTileHtml(product) {
  return [
    '<div class="product-tile" data-product-id="' +
      escHtml(product.id) +
    ...
    '    <div class="product-tile-name">' +
      escHtml(pt(product, "name")) +
      "</div>",
    '    <div class="product-tile-price">' +
      formatPrice(product.price) +
      "</div>",
    '    <div class="product-tile-symbol">' +
      escHtml(pt(product, "symbolMeaning")) +
      "</div>",
    "  </div>",
    "</div>",
  ].join("\n");
...
function renderHome() {
  ...
  app.innerHTML = [
    ...
    '  <video class="hero-video hero-video--mobile" autoplay muted loop playsinline preload="metadata" poster="images/hero-mobile-poster.jpg?v=14" aria-hidden="true"><source src="images/hero-mobile.mp4?v=14" type="video/mp4"></video>',
    ...
    '    <a href="#/shop" class="btn btn--large">' +
      escHtml(t("home_cta_btn")) +
      "</a>",
    "  </section>",
...
function renderProduct(productId) {
  ...
  var descHtml = pt(product, "description")
    .map(function (p) {
      return "<p>" + escHtml(p) + "</p>";
    })
    .join("");

  var thumbsHtml = product.images
    .map(function (src, i) {
      return [
  ...
  app.innerHTML = [
    ...
    '    <a href="#/shop" class="product-back-btn">← ' +
      escHtml(t("back_to_shop")) +
      "</a>",
    "  </div>",
    ...
    '        <button class="btn btn--full" id="pp-add-btn">' +
      escHtml(t("product_add")) +
      "</button>",
    '        <button class="btn btn--ghost btn--full" id="pp-tryon-btn">' +
      escHtml(t("product_tryon")) +
      "</button>",
    '        <p class="product-actions-note">' +
      escHtml(t("product_leave")) +
      "</p>",
    "      </div>",

    '      <div class="pp-symbol">',
    '        <p class="product-details-symbol">' +
      escHtml(product.symbolName) +
      "</p>",
    '        <p class="product-details-meaning">' +
      escHtml(pt(product, "symbolMeaning")) +
      "</p>",
    "      </div>",
    ...
    '          <div class="product-meta-label">' +
      escHtml(t("product_materials")) +
      "</div>",
    '          <div class="product-meta-value">' +
      escHtml(pt(product, "materials")) +
      "</div>",
    "        </div>",
    '        <div class="product-meta-item">',
    '          <div class="product-meta-label">' +
      escHtml(t("product_size")) +
      "</div>",
    '          <div class="product-meta-value">' +
      escHtml(pt(product, "size")) +
      "</div>",
    "        </div>",
    "      </div>",

    '      <div class="product-tactile">',
    '        <div class="product-tactile-label">' +
      escHtml(t("product_tactile")) +
      "</div>",
    '        <div class="product-tactile-text">' +
      escHtml(pt(product, "tactile")) +
      "</div>",
    "      </div>",
...
function renderContact() {
  ...
  app.innerHTML = [
    ...
    '        <label for="cf-name">' +
      escHtml(t("contact_name_label")) +
      ' <span style="opacity:.4">' +
      escHtml(t("contact_name_opt")) +
      "</span></label>",
    '        <input type="text" id="cf-name" name="name" placeholder="' +
      escHtml(t("contact_name_placeholder")) +
      '">',
    "      </div>",
    '      <div class="form-group">',
    '        <label for="cf-email">' +
      escHtml(t("contact_email_label")) +
      "</label>",
    '        <input type="email" id="cf-email" name="email" placeholder="' +
      escHtml(t("contact_email_placeholder")) +
      '" required>',
    "      </div>",
    '      <div class="form-group">',
    '        <label for="cf-message">' +
      escHtml(t("contact_message_label")) +
      "</label>",
    '        <textarea id="cf-message" name="message" placeholder="' +
      escHtml(t("contact_message_placeholder")) +
      '"></textarea>',
    "      </div>",
    '      <div class="form-submit">',
    '        <button type="submit" class="btn btn--large">' +
      escHtml(t("contact_send")) +
      "</button>",
    "      </div>",
    ...
    "      <p>" +
      escHtml(t("contact_or_email")) +
      ' <a href="mailto:hello@sicc.com">hello@sicc.com</a></p>',
    "      <p>" +
      escHtml(t("contact_instagram")) +
      ' <a href="https://instagram.com/sicc.jewelry" target="_blank" rel="noopener">@sicc.jewelry</a></p>',
    "    </div>",
    "  </div>",
...
function renderNotFound() {
  ...
  app.innerHTML = [
    ...
    '      <a href="#/shop" class="btn">' +
      escHtml(t("notfound_shop")) +
      "</a>",
    '      <a href="#/" class="btn btn--ghost">' +
      escHtml(t("notfound_home")) +
      "</a>",
    "    </div>",
    "  </div>",
    '  <span class="not-found-bottom">' +
      escHtml(t("notfound_tagline")) +
      "</span>",
    "</div>",
    footerHtml(),
  ].join("\n");
...
function renderCheckout() {
  ...
  app.innerHTML = [
    ...
    summaryRows,
    '      <div class="checkout-total"><span>' +
      escHtml(t("cart_total") || "Total") +
      "</span><span>" +
      formatPrice(total) +
      "</span></div>",
    "    </div>",
    ...
    '      <div class="form-group"><label>' +
      escHtml(t("checkout_email_label")) +
      '</label><input type="email" required placeholder="' +
      escHtml(t("contact_email_placeholder")) +
      '"></div>',

    "      <h2>" + escHtml(t("checkout_shipping_h2")) + "</h2>",
    '      <div class="form-group"><label>' +
      escHtml(t("checkout_name_label")) +
      '</label><input type="text" required placeholder="Your Name"></div>',
    '      <div class="form-group"><label>' +
      escHtml(t("checkout_address_label")) +
      '</label><input type="text" required placeholder="Street address"></div>',
    '      <div class="checkout-row">',
    '        <div class="form-group"><label>' +
      escHtml(t("checkout_city_label")) +
      '</label><input type="text" required placeholder="City"></div>',
    '        <div class="form-group"><label>' +
      escHtml(t("checkout_postal_label")) +
      '</label><input type="text" required placeholder="10000"></div>',
    "      </div>",
    '      <div class="form-group"><label>' +
      escHtml(t("checkout_country_label")) +
      '</label><input type="text" required placeholder="' +
      escHtml(t("checkout_country_placeholder")) +
      '"></div>',

    "      <h2>" + escHtml(t("checkout_payment_h2")) + "</h2>",
    '      <p style="font-size:13px;color:#888;font-weight:300;padding:8px 0">' +
      t("checkout_payment_note") +
      "</p>",

    '      <div class="form-submit" style="margin-top:16px">',
    '        <button type="submit" class="btn btn--large btn--full">' +
      escHtml(t("checkout_btn")) +
      "</button>",
    "      </div>",
    "    </form>",
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      document.querySelector(".checkout-inner").innerHTML = [
        ...
        '  <h1 style="font-size:48px;font-weight:900;letter-spacing:-0.03em;margin-bottom:16px">' +
          escHtml(t("checkout_success_h1")) +
          "</h1>",
        '  <p style="font-size:16px;font-weight:300;color:#666;margin-bottom:32px">' +
          escHtml(t("checkout_success_p")) +
          "</p>",
        '  <a href="#/" class="btn">' +
          escHtml(t("checkout_success_btn")) +
          "</a>",
        "</div>",
      ].join("");
    });
